    ...
    const rest = new RestMemory(scanLogsService.cache.pipe(
      map(ls => {
        ...
        for (const l of ls) {
          ...
          const dateKey = new Date(l.readmeCommit?.author?.date).toISOString().slice(0,7)
          if (!(dateKey in vs)) {
            vs[dateKey] = {
              updates: 0,
              activeStudies: 0,
        ...
        // return {}
        return Object.entries(vs).reduce((acc, [k, v]) => {
          acc[k] = {
            date: k,
            updatesLast30: v.updates,
            activeStudiesLast30: v.activeStudies,
            newStudiesLast30: v.newStudies,
            studiesStartedLast30: v.studiesStarted,
            designFinalizedLast30: v.designFinalized,
            resultsAvailableLast30: v.resultsAvailable,
            studiesCompletedLast30: v.studiesCompleted,
          }
          return acc
