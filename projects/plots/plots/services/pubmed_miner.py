...
def getLastUpdatedCitations(containerName):
    ...
    """
    container = db.init_cosmos(containerName)
...
def makeCSVJSON(table, containerChosen: str, forUpdate: bool):
    ...
    """
    container = db.init_cosmos( 'pubmed')
    container_ignore = db.init_cosmos( 'pubmed_ignore')
    container_chosen = db.init_cosmos( containerChosen)
    if(forUpdate):
    ...
    ignore_list = db.getExistingIDandSearchStr( 'pubmed_ignore')[0]
...
def moveItemToIgnoreContainer( pmIDList, fromContainerName: str, toContainerName: str):
    ...
    """
    fromContainer = db.init_cosmos( fromContainerName)
    toContainer = db.init_cosmos( toContainerName)
    for i in range(len(pmIDList)):
        pmID = pmIDList[i]
        #check if the article exists in the current container.
        checkID = db.getExistingIDandSearchStr(fromContainerName)[0]
...
def identifyNewArticles(table):
    ...
    """
    trueArticles = db.getExistingIDandSearchStr('pubmed')
    ignoreArticles = db.getExistingIDandSearchStr('pubmed_ignore')
...
def update_data():
    ...
    lastUpdated = db.getTimeOfLastUpdate()[0:2] + db.getTimeOfLastUpdate()[5:10]

    if(lastUpdated == dateMY):
        print("Already updated this month on " + db.getTimeOfLastUpdate())
        print("Identifying new articles...")
        #check if an update has already been performed today
        if(db.getTimeOfLastUpdate() != str("" + date.datetime.now().strftime("%m-%d-%Y"))):
